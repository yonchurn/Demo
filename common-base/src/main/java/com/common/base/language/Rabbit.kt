...
/**
 * 缅甸语言 mm3(unicode)和zawgyi转换
 */
object Rabbit {

    private var uni2ZgRuleJSONArray: JSONArray? = null
    private var zg2uniRuleJSONArray: JSONArray? = null
    ...
    fun uni2zg(input: String?): String? {
        ...
        if (uni2ZgRuleJSONArray == null) {
            try {
                ...
                uni2ZgRuleJSONArray = JSONArray(uni2ZgRule)
            } catch (e: JSONException) {
                e.printStackTrace()
            }
        }
        return replaceWithRule(uni2ZgRuleJSONArray, input!!)
    }
    ...
    fun zg2uni(input: String?): String? {
        ...
        if (zg2uniRuleJSONArray == null) {
            try {
                ...
                zg2uniRuleJSONArray = JSONArray(zg2uniRule)
            } catch (e: JSONException) {
                e.printStackTrace()
            }
        }
        return replaceWithRule(zg2uniRuleJSONArray, input!!)
    }

    private fun replaceWithRule(rule_array: JSONArray?, output: String): String {
        var result = output
        if (rule_array == null) return result
        try {
            val maxLoop = rule_array.length()

            //because of JDK 7 bugs in Android
            result = result.replace("null", "\uFFFF\uFFFF")
            for (i in 0 until maxLoop) {
                val obj = rule_array.getJSONObject(i)
                result = result.replace(obj.getString("from").toRegex(), obj.getString("to"))
                result = result.replace("null", "")
            }
        } catch (e: JSONException) {
            e.printStackTrace()
